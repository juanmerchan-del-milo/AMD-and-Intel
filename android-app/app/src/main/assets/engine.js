...
function empty(){return {cpu:null,gpu:null,ram:null,storage:[],motherboard:null,psu:null,cooler:null,case:null}}
...
function check(build,db=root.PC_CATALOG){const b=resolve(build,db),errors=[],warnings=[],p=power(build,db);const {cpu:c,gpu:g,ram:r,motherboard:m,case:h,psu:s,cooler:f,storage:d}=b;const add=(list,key)=>{if(!list.includes(key))list.push(key)};
 ...
 if(c&&!g&&c.igpu==='No')add(errors,'videoBad');
 ...
 return {errors,warnings,missing,power:p,complete:!missing.length,ok:!errors.length&&!missing.length};
}
const relevant={cpu:['mobileCpu','socketBad','chipsetBad','ramBad','coolerSocket','videoBad','psuBad'],gpu:['gpuBad','psuBad'],ram:['ramBad','formBad','ramSlots','ramLimit'],storage:['m2Bad','sataBad','bayBad'],motherboard:['socketBad','chipsetBad','ramBad','formBad','ramSlots','ramLimit','caseBad','m2Bad','sataBad'],psu:['psuBad','psuFormBad'],cooler:['coolerBad','radiatorBad','coolerSocket'],case:['caseBad','psuFormBad','gpuBad','coolerBad','radiatorBad','bayBad']};
function compatibleCandidate(build,category,item){const test=structuredCloneSafe(build);test[category]=category==='storage'?[item.id]:item.id;return !check(test).errors.some(x=>relevant[category].includes(x))}
function structuredCloneSafe(value){return JSON.parse(JSON.stringify(value))}
function validateBuild(value,db=root.PC_CATALOG){if(!value||typeof value!=='object'||Array.isArray(value))throw Error('build');const out=empty();for(const cat of categories){if(cat==='storage'){if(!Array.isArray(value.storage)||value.storage.length>12)throw Error('storage');out.storage=value.storage.map(id=>{if(typeof id!=='string'||!db.storage.some(x=>x.id===id))throw Error('drive');return id})}else{if(value[cat]!==null&&(typeof value[cat]!=='string'||!db[cat].some(x=>x.id===value[cat])))throw Error(cat);out[cat]=value[cat]}}return out}
